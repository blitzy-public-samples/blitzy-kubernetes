...
- ✅ **Quality gates:** `gofmt`/`go vet`/`shellcheck`/YAML all clean (independently re-verified); the V1–V8 unit + integration security suites green (the one non-passing test, `TestNodeRestrictionServiceAccountAudience`, is a pre-existing, out-of-scope, environment-blocked upstream test unrelated to V1–V8 — see §3); control-plane binaries build and run; every artifact carries inline tech-spec/AAP citations.
...
> These are **deployment-execution** items, not code defects. No compilation failures and no V1–V8 test failures exist on this branch; the only non-passing test is the pre-existing, out-of-scope `TestNodeRestrictionServiceAccountAudience` (unrelated to V1–V8), which is environment-blocked — its in-process kube-controller-manager cannot launch where the KCM delegated-auth client lacks RBAC to read `kube-system/extension-apiserver-authentication` (see §3).
...
| Integration — Auth (V1/V2/V4/V5/V7) | Go `testing` + apiserver | 49 | 48 | 1 | N/A | 48 of 49 top-level tests pass (~208s): all V1/V2/V4/V5/V7 hardening + acceptance tests (original no-wildcard, privileged-pod-rejection, bound/audienced-token, cross-node-denial, plus the 6 new hardening functions in the following rows) are green. The single failure, `TestNodeRestrictionServiceAccountAudience`, is a **pre-existing upstream test (not part of V1–V8)** whose in-process kube-controller-manager cannot launch in this environment — its delegated-auth client lacks RBAC to read `kube-system/extension-apiserver-authentication` — an environment limitation, not a code defect or hardening regression (out of scope per AAP §0.8.2) |
...
**Aggregate:** This task adds **12 negative-path/boundary regression tests across V1–V8** (detailed in the six new-test rows above) — including the new V5 fail-closed admission-webhook file (`TestAdmissionWebhookFailClosedDeniesUnreachable`) and the V8 etcd-mTLS fail-closed table (`TestConfigureEtcdParamsFailClosed`) — plus one subprocess helper (`runConfigureEtcdParamsExitCode`), spanning **8 updated + 3 created** test files. Every new test is green and strictly additive (0 removed lines from the task base, no new `init()`/globals), and all pre-existing acceptance tests (including the earlier per-weakness regression tests for V1/V2/V3/V4/V6/V7 and the V6 audit-policy unit test) remain unchanged and green. The `secrets`, `controlplane/audit`, and `gci` packages report **0 failures**; the `auth` package passes **48 of 49** top-level tests — the sole non-passing test, `TestNodeRestrictionServiceAccountAudience`, is a **pre-existing upstream test (not part of V1–V8)** whose in-process kube-controller-manager cannot launch where its delegated-auth client lacks RBAC to read `kube-system/extension-apiserver-authentication` (an environment limitation, not a code defect — out of scope per AAP §0.8.2; see §3). `Coverage %` remains **`N/A`** by convention.
...
**Success metrics.** `kube-bench` moves the RBAC, Pod Security, etcd-encryption, audit, and NodeRestriction controls to PASS; `etcdctl` shows `k8s:enc:` ciphertext for Secrets; privileged pods are rejected in enforced namespaces; the V1–V8 hardening and acceptance tests stay green and the auth integration suite passes 48/49 top-level tests (the sole exception is the pre-existing, out-of-scope, environment-blocked `TestNodeRestrictionServiceAccountAudience` — see §3).

...
| Integration — Auth (V1/V2/V4/V5/V7) | Go `testing` + apiserver | 49 | 49 | 0 | N/A | Full `test/integration/auth` package: 49 top-level tests, 0 failures, ~208s (the original no-wildcard, privileged-pod-rejection, bound/audienced-token, and cross-node-denial acceptance tests plus the 6 new hardening functions in the following rows) |

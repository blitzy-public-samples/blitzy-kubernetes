...
- ✅ **Quality gates:** `gofmt`/`go vet`/`shellcheck`/YAML all clean (independently re-verified); the V1–V8 unit + integration security suites green, and the full `auth` integration suite passes 49/49 top-level tests when run with the pod's in-cluster env vars unset (`TestNodeRestrictionServiceAccountAudience`, a pre-existing upstream test unrelated to V1–V8, is sensitive to those env vars — see §3/§9.5); control-plane binaries build and run; every artifact carries inline tech-spec/AAP citations.
...
> These are **deployment-execution** items, not code defects. No compilation failures and no V1–V8 test failures exist on this branch, and the full `auth` integration suite passes 49/49 top-level tests. The one environment-sensitive test, the pre-existing `TestNodeRestrictionServiceAccountAudience` (unrelated to V1–V8), passes once integration tests are run with the pod's in-cluster env vars unset; if they are left set inside a Kubernetes pod, its in-process kube-controller-manager's delegated-auth client picks up the pod's mounted SA token and is denied reading `kube-system/extension-apiserver-authentication` — a test-invocation-environment effect, not a code defect (see §3/§9.5).
...
| Integration — Auth (V1/V2/V4/V5/V7) | Go `testing` + apiserver | 49 | 49 | 0 | N/A | All 49 top-level tests pass (~224s): all V1/V2/V4/V5/V7 hardening + acceptance tests (original no-wildcard, privileged-pod-rejection, bound/audienced-token, cross-node-denial, plus the 6 new hardening functions in the following rows) are green. Note on `TestNodeRestrictionServiceAccountAudience` (a **pre-existing upstream test, not part of V1–V8** — the only auth test that starts an in-process kube-controller-manager): when the suite is run **inside a Kubernetes pod**, the KCM delegated-auth client picks up the pod's mounted service-account token via `rest.InClusterConfig()` (`system:serviceaccount:default:default`) and is denied reading `kube-system/extension-apiserver-authentication`. This is a test-**invocation-environment** effect, not a code defect or hardening regression; it is resolved by running integration tests with the pod's in-cluster env vars unset (`KUBERNETES_SERVICE_HOST`/`KUBERNETES_SERVICE_PORT`), the standard way to run Kubernetes in-process integration tests inside a pod — after which this test passes and the suite is 49/49 (re-verified: `ok ~224s`) |
...
| Integration — Secrets encryption boundaries (V3) | Go `testing` + etcd | 2 | 2 | 0 | N/A | `TestEncryptionKMSv2CachesizeRejectedAtStartup` (rejects `cachesize` under KMS `apiVersion: v2` at apiserver startup — "cachesize is not supported in v2"); `TestEncryptionIdentityProviderLastFallback` (identity-provider-last fallback: seeds a pre-existing identity/plaintext Secret via a first identity-storage server, then proves the aesgcm-first/identity-last server reads it back through the identity provider at the end of the decrypt chain — genuine fallback decryption — while new writes are aesgcm-first with the plaintext canary absent and transparently decrypted) |
...
**Aggregate:** This task adds **12 negative-path/boundary regression tests across V1–V8** (detailed in the six new-test rows above) — including the new V5 fail-closed admission-webhook file (`TestAdmissionWebhookFailClosedDeniesUnreachable`) and the V8 etcd-mTLS fail-closed table (`TestConfigureEtcdParamsFailClosed`) — plus one subprocess helper (`runConfigureEtcdParamsExitCode`), spanning **8 updated + 3 created** test files. Every new test is green and strictly additive (0 removed lines from the task base, no new `init()`/globals), and all pre-existing acceptance tests (including the earlier per-weakness regression tests for V1/V2/V3/V4/V6/V7 and the V6 audit-policy unit test) remain unchanged and green. The `secrets`, `controlplane/audit`, and `gci` packages report **0 failures**; the `auth` package passes **all 49** top-level tests. The one test that is sensitive to the run environment, `TestNodeRestrictionServiceAccountAudience` (a **pre-existing upstream test, not part of V1–V8** — the only auth test that starts an in-process kube-controller-manager), passes once the suite is invoked with the pod's in-cluster service-account env vars unset (`KUBERNETES_SERVICE_HOST`/`KUBERNETES_SERVICE_PORT`); when they are left set inside a Kubernetes pod, the KCM delegated-auth client picks up the pod's mounted SA token (`system:serviceaccount:default:default`) via `rest.InClusterConfig()` and is denied reading `kube-system/extension-apiserver-authentication` — a test-invocation-environment effect, not a code defect or hardening regression (see §3 and §9.5). `Coverage %` remains **`N/A`** by convention.
...
**Success metrics.** `kube-bench` moves the RBAC, Pod Security, etcd-encryption, audit, and NodeRestriction controls to PASS; `etcdctl` shows `k8s:enc:` ciphertext for Secrets; privileged pods are rejected in enforced namespaces; the V1–V8 hardening and acceptance tests stay green and the auth integration suite passes 49/49 top-level tests when run with the pod's in-cluster env vars unset (the pre-existing `TestNodeRestrictionServiceAccountAudience` is sensitive to those env vars — see §3/§9.5).
...
# 5) Security integration tests (need etcd on PATH; ~minutes):
#    Ensure the vendored etcd is discoverable by the test harness:
export PATH="$PWD/third_party/etcd:$PATH"
#    When running INSIDE a Kubernetes pod, unset the pod's in-cluster
#    service-account env vars so the in-process kube-controller-manager
#    (used by TestNodeRestrictionServiceAccountAudience) does not pick up the
#    pod's mounted SA token via rest.InClusterConfig(); otherwise that one
#    pre-existing upstream test fails at KCM startup (see §3). Harmless to set
#    even outside a pod:
env -u KUBERNETES_SERVICE_HOST -u KUBERNETES_SERVICE_PORT \
    -u KUBERNETES_SERVICE_PORT_HTTPS -u KUBERNETES_PORT \
    -u KUBERNETES_PORT_443_TCP -u KUBERNETES_PORT_443_TCP_ADDR \
...
| `test/integration/secrets/encryption_test.go` | CREATE | V3 | Ciphertext-at-rest assertion; + cachesize-under-KMS-v2 rejection & identity-last fallback decryption of a pre-existing plaintext Secret (`TestEncryptionKMSv2CachesizeRejectedAtStartup`, `TestEncryptionIdentityProviderLastFallback`) |
